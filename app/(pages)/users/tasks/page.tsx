...
import { defaultValue, sortTask, usersTaskRow } from "@/constants/task"
...
import { createTasks, fetchTasks, patchTeamsProjectTasks } from "@/api/tasks"
...
const TasksPage = () => {
    ...
    const {indexes, currTaskDetails, setCreatetask, setFormTask, groups} = useContext(connectContext)
    //function de trie
    ...
    useEffect(() => {
        function handleFetch () {
            fetchTasks().then((res) => {
                task.setValue(res.data)
                storeTasks.current = res.data
            }).catch((error) => {
                setGetter((prev) => [...prev, error])
            })
        }
    ...
    function patchProject () {
        if (indexes !== null) {
            ...
            patchTeamsProjectTasks(groups, task.value[indexes].id)
            .catch((error) => {
                setGetter((prev) => [...prev, error])
            })
    ...
    function submitTask (formTask: Tasks) {
       try {
        TaskSchema.parse(formTask)
        createTasks(formTask)
        .then((res) => {
