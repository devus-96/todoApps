...
export const Popup = () => {
    // state call
    const [clock, setClock] = useState('00:00AM')
    const [clockEnd, setClockEnd] = useState('00:00AM')
    ...
    return <div className="fixed top-0 bottom-0 left-0 right-0 bg-popup flex items-center justify-center font-[family-name:var(--font-jetBrains-mono)]">
            <Clock 
                ...
                show={show} 
                type={type}
                setClockEnd={setClockEnd}
            />
            {showCalendar && <Calendar value={currentDate} onChange={setCurrentDate} setTrigger={setShowCalendar}/>}
            <div className="w-[500px] h-auto space-y-4 bg-white rounded px-8 py-8">
                ...
                <input onClick={() => setShowCalendar(true)} type="button" value={`${format(currentDate, "ccc dd LLLL yyyy")}`} className="bg-gray-200 rounded px-4 py-2 cursor-pointer"/>
                <div className="w-full flex items-center space-x-4">
                    <button type="button" onClick={() => {
                        setType('start')
                        setShow(true)
                    }} className="bg-gray-200 rounded px-4 py-2 cursor-pointer">{clock}</button>
                    <p>-</p>
                    <button type="button" onClick={() => {
                        setType('end')
                        setShow(true)
                    }} className="bg-gray-200 rounded px-4 py-2 cursor-pointer">{clockEnd}</button>
                </div>
                <div>
