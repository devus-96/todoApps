...
export const FormLogin = () => { 
    ...
    return <form action={(formData) => {
        postDatas('/login/user', formData)
    }} className="bg-white w-[370px] absolute left-1/2 translate-x-[-50%] px-8 pt-2 pb-8 mt-4 space-y-10 rounded-2xl">
                    ...
                    <InputPasswrld placeholder="password must have least 8 characteres)" name="password" handle={() => {}}/>
                    ...
                    <div>
                        <button className="w-full py-[11px] rounded-full text-sm bg-white border border-black cursor-pointer hover:bg-gray-400 hover:text-white focus:ring-2 focus:ring-gray-300 active:bg-gray-600 transition-colors duration-300 flex-center gap-4 hover:border-gray-400"><FaGoogle size={20} /> Sign in with Google</button>
                        <p className="text-sm text-gray-500 mt-4">Don't have an account ? <Link href='/login' className="text-[#9eabe4] underline ">sign up</Link></p>
                    </div>
            </form>
}